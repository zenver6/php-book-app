...
<?php if ($questions->isEmpty()): ?>
    <div class="card mb-2">
        <div class="card-body">
            <h5 class="cart-title text-center">表示できる質問がありません。</h5>
        </div>
    </div>
<?php else:?>
    ...
    <div class="card mb-2">
        <div class="card-body">
            <h5 class="card-title">
                <i class="fas fa-user-circle"></i> <?= 'タロウ' ?>
            </h5>
            <p class="card-text"><?= nl2br(h($question->body)) ?></p>
            <p class="card-subtitle mb-2 text-muted">
                <small><?= h($question->created) ?></small>
                <br>
                <small><i class="fas fa-comment-dots"></i> <?= $this->Number->format($question->answered_count) ?></small>
            </p>
